...
namespace combi				// Размещение
{
	...
	unsigned __int64 factt(unsigned __int64 x) { return (x == 0) ? 1 : (x*factt(x - 1)); };

	unsigned __int64  accomodation::count() const
	{

		return (this->n >= this->m) ?
			factt(this->n) / factt(this->n - this->m) : 0;
	};
}

#define NINF  ((short)0x8000)
namespace combi			//Перестановка
{
	...
	unsigned __int64 facttt(unsigned __int64 x) { return (x == 0) ? 1 : (x*facttt(x - 1)); };
	unsigned __int64 permutation::count() const { return facttt(this->n); };
}
